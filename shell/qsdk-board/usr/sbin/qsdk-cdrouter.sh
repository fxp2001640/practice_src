...
cdrouter_cfg(){
	...
	#DHCP Server Options
	uci set dhcp.lan.dhcp_option='46,8 47 19,0 44,0.0.0.0 45,0.0.0.0'
	#UPnP	
...
#@
#@cdrouter_static
#@example : cdrouter_static
cdrouter_static(){
	uci_set_static_wan eth0 192.168.0.2 192.168.0.1 "1.1.1.1 1.1.1.2" 0
	cdrouter_cfg
	cdrouter_restart
}
...
#@
#@cdrouter_pppoe
#@example : cdrouter_pppoe
cdrouter_pppoe(){
	uci_set_pppoe_wan eth0 qacafe qacafe123 0
	cdrouter_cfg
	cdrouter_restart
}

#@
#@cdrouter <host> <file> [force]
#@cdrouter 192.168.1.10 uci-config-dhcp
#@cdrouter 192.168.1.10 uci-config-pppoe
cdrouter(){
	local host=$1
	local file=$2
	local force=$3

	if [ "$force" == "force" ] || ! [ -e "/etc/$file.sh" ]; then
		cd /tmp
		tftp-hpa $host -c get $file
		echo '#!/bin/sh' > /etc/$file.sh
		echo " " >> /etc/$file.sh
		cat ./$file >> /etc/$file.sh
		rm -f ./$file
	fi

	chmod a+x /etc/$file.sh
	/etc/$file.sh
}
